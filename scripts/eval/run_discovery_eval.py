...
def score(example: dict, status: str, apis: set[str]) -> str:
    ...
    gold = example["gold_api"]
    if status == "fall_through":
        return MISSED
    return ROUTED if gold in apis else MISROUTE
...
def main(argv: list[str] | None = None) -> int:
    ...
    bad = sorted({ex["gold_api"] for ex in examples
                  if ex["gold_api"] and ex["gold_api"] not in catalog_apis})
    if bad:
    ...
    print(f"=== discovery eval: strategy={args.strategy}  n={len(examples)} ===\n")
